...
{
    SqlConnection con = new SqlConnection("server=.;uid=sa;pwd=admin123;database=lever_Deduplication");
    SqlCommand cmd;
    SqlDataAdapter adp;
    SqlDataReader rd;
    ...
    public string coomunication(int pl)
    {
        string possibles = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZasdfghhhkjkdhwdvwfywdsbtedftftrtyfdcqdvwcbcdjc";
        char[] passwords = new char[pl];
        Random rd = new Random();
        for (int i = 0; i < pl; i++)
        {
    ...
    {
        {
            string possibles = "23456789098765432134567463143465823451234567890";
            char[] passwords = new char[pl];
            Random rd = new Random();
            for (int i = 0; i < pl; i++)
            {
